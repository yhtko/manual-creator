...
7. 説明文を編集し、Word / HTML / PowerPoint を出力します。
...
GIF変換やMP4変換はブラウザ負荷とファイルサイズが大きいため、現時点では行いません。録画済みのWebMはクリック時点のスクリーンショット抽出素材として使用します。
...
`sharepoint_paste.md` の本文をSharePointページへ貼り付けた後、画像ファイルを該当箇所へ手動で配置します。動画は出力対象にせず、録画ファイルはクリック時点のスクリーンショット抽出素材として使用します。

ブラウザ内MP4変換は重く、SharePoint上での再生互換性も安定しないため、初期版では行いません。Chrome拡張機能の録画はWebMを標準にしています。
